...
function generateReactRule(styleObj, containerColorMap, mixin) {
  ...
  Object.keys(styleObj).forEach(function(prop) {
    ...
    if (
      REACT_RULES_WITH_COLOR.includes(prop) &&
      styleObj[prop] in containerColorMap
    ) {
...
function getStyleguideColorTexts(colorFormat, colors) {
  return colors.map(color => {
    var colorStyleObject = getColorStringByFormat(color, colorFormat);
...
function getStyleguideTextStylesCode(options, containerAndType, textStyles) {
  var textStylesObj = generateStyleguideTextStylesObject(
    options,
    containerAndType,
    textStyles
  );
  ...
  var processedTextStyles = textStylesStr
    .replace(/"(.+)":/g, "$1:")
    .replace(/: "colors\.(.*)"/g, ": colors.$1");
...
function getLayerCode(containerAndType, layer, options) {
  ...
  var containerColors = getResources(
    container,
    type,
    useLinkedStyleguides,
    "colors"
...
export { getStyleguideColorsCode, getStyleguideTextStylesCode, getLayerCode };
