...
import { NUMERICAL_FONT_BOLD, NUMERICAL_FONT_NORMAL } from "../constants";
...
function generateLayerStyleObject({
  ...
  if (layerType === "text" && layer.defaultTextStyle) {
    ...
    layer.fills.length &&
    !layerHasGradient(layer) &&
    !layerHasBlendMode(layer)
  ) {
...
function generateTextStyleStyleObject({
  ...
  overrideLayerStyle =
    layerStyle &&
    layerStyle["font-weight"] &&
    layerStyle["font-weight"] !== "normal";
  ...
  overrideLayerStyle =
    layerStyle &&
    layerStyle["font-style"] &&
    layerStyle["font-style"] !== "normal";
  if (
    textStyle.fontStyle &&
    (textStyle.fontStyle !== "normal" || defaultValues || overrideLayerStyle)
  ) {
    styleProperties.fontStyle =
      textStyle.fontStyle === "oblique" ? "italic" : textStyle.fontStyle;
  }

  if (textStyle.lineHeight) {
    styleProperties.lineHeight = round(
      textStyle.lineHeight / densityDivisor,
      1
    );
  }
  ...
  overrideLayerStyle =
    layerStyle &&
    layerStyle["letter-spacing"] &&
    layerStyle["letter-spacing"] !== "normal";
  if (textStyle.letterSpacing) {
    var PRECISION = 2;
    styleProperties.letterSpacing = round(
      textStyle.letterSpacing / densityDivisor,
      PRECISION
    );
  } else if (defaultValues || overrideLayerStyle) {
  ...
  if (textStyle.color) {
    styleProperties.color = getColorStringByFormat(
      textStyle.color,
      colorFormat
    );
  }
...
function generateStyleguideTextStylesObject(
  options,
  containerAndType,
  textStyles
) {
  ...
  return textStyles.reduce((styles, ts) => {
    var tsParams;
    if (ts.color) {
      var containerColor = container.findColorEqual(
        ts.color,
        options.useLinkedStyleguides
      );
